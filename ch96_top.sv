...
(
	...
	//////////// LEDs //////////
	output        LED_YELLOW,  // D9, PIN_Y19 (video-active)
	output        LED_BLUE     // D10, PIN_Y20 (always-on)
);
	...
	// The four user LEDs (D5-D8) are HPS GPIO14/22/25/32 (I11/H11/G10/F11),
	// muxed to LoanIO in the hps-ch96 handoff, so the FPGA drives them through
	// the loan_io bus. Board LEDs are active-high (1 = lit, as exercised by
	// the reference design's blink test), matching the sys_top LED vector.
	wire [7:0] led;
	wire [66:0] loan_io_in;
	wire [66:0] loan_io_out = {1'b0, 33'd0, led[6], 6'd0, led[4], 2'd0, led[2], 7'd0, led[0], 14'd0};
	wire [66:0] loan_io_oe  = {1'b0, 33'd0, 1'b1, 6'd0, 1'b1, 2'd0, 1'b1, 7'd0, 1'b1, 14'd0};

	cyclonev_hps_interface_loan_io loan_io_inst
	(
		.loanio_in(loan_io_in),
		.loanio_out(loan_io_out),
		.loanio_oe(loan_io_oe)
	);

	// RF LEDs: yellow = video-active, blue = always-on. Driven from LED[1]/[3]
	// so cores/apps can still override them via UIO_LEDS.
	assign LED_YELLOW = led[1];
	assign LED_BLUE   = led[3];
